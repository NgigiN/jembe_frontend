...
class _FakeCostCategoryRemoteDataSource
    implements CostCategoryRemoteDataSource {
...
void main() {
  ...
  group('push — create', () {
    test('calls addCostCategory with client_uuid and marks the local row '
        'synced WITHOUT assigning a server id', () async {
      await local.upsert(
        _category(
          clientUuid: 'cu-1',
          name: 'Seeds',
    ...
    test('a false return from addCostCategory leaves the row pending (not '
        'reached by the real data source, but handled defensively)', () async {
  ...
  group('push — delete', () {
    test('calls deleteCostCategory with the server id then hard-deletes '
        'locally', () async {
      await local.upsert(
        _category(clientUuid: 'cu-1', id: 'server-1'),
        pending: false,
  ...
  group('pull', () {
    test(
      ...
      () async {
        ...
        expect(rows.map((r) => r.clientUuid).toSet(), {
          'srv:server-1',
          'srv:server-2',
        });
        expect(
    ...
    test('PRESERVES a pending local row (an offline create/delete awaiting '
        'push) untouched by the full re-fetch', () async {
      await local.upsert(
        _category(
    ...
    test('is idempotent: re-running pull with the same server rows converges '
        'to the same final state', () async {
      remote.getCostCategoriesResult = [
        _category(
          clientUuid: '',
          id: 'server-1',
