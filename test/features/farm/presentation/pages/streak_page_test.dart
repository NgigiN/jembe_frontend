...
void main() {
  ...
  testWidgets(
    'shows the level label and an explanation when everything is stale',
    (tester) async {
      final herd = _herd('h1');
      await _pumpStreakPage(
        tester,
  ...
  testWidgets('shows the weekly streak count with encouragement', (
    tester,
  ) async {
    final herd = _herd('h1');
    await _pumpStreakPage(
      ...
      await _pumpStreakPage(
        ...
        activities: [
          _activityFor(
            'animal',
            'stale',
            _now.subtract(const Duration(days: 45)),
          ),
          _activityFor(
            'plant',
            'fresh',
  ...
  testWidgets('does not fire when the level never reaches Thriving', (
    tester,
  ) async {
    final calls = <MethodCall>[];
    TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
